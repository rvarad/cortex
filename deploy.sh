...
# ==========================================
# 2b. SWAG / fail2ban config
# ==========================================
# fail2ban must never ban Cloudflare. Without this file SWAG seeds its own
# jail.local, which bans the CDN's edge IPs (nginx sees no real client IP) and
# takes the site down for everyone -- 17 days of ~80% downtime.
# SWAG only seeds its default if this file is ABSENT, so place it before `up`.
echo "🛡️  Installing fail2ban jail config..."
sudo mkdir -p swag_config/fail2ban
sudo cp swag/jail.local swag_config/fail2ban/jail.local
...
docker compose -f docker-compose.prod.yaml --env-file ../.env restart swag
