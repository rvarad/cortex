...
// private final RateLimiter inferenceRateLimiter = RateLimiter.create(15 /
// 60.0);
...
// try {
// log.info("Uploading file: {} to gemini using files api, attempt: {}/{}",
...
// } catch (Exception e) {
// log.warn("Upload attempt {}/{} failed for {}, Error: {}", attempt,
// maxRetries, objectName, e.getMessage());
...
// } catch (Exception e) {
// throw new RuntimeException("Failed to upload file to gemini using files api:
// " + objectName + "due to: " + e);
// } finally {
// try {
...
// if (attempts++ > 30) {
// throw new RuntimeException("Timed out waiting for file to process: " +
// objectName);
// }
// if (currentFile.state().isPresent() &&
// "FAILED".equals(currentFile.state().get().toString())) {
// throw new RuntimeException("File failed to process: " + objectName);
// }
// log.info("Waiting for file to be active, current state: {} for object: {}",
// currentFile.state(), objectName);
// Thread.sleep(2000);
...
// genAiClient.models.generateContent("gemini-3-flash-preview",
