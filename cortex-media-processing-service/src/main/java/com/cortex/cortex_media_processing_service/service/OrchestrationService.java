...
// @KafkaListener(topics = "media-chunk-uploaded", groupId =
// "cortex-media-processing-group", concurrency = "3", properties = {
// "max.poll.records=1",
// "max.poll.interval.ms=300000"
// })
// public CompletableFuture<Void> handlefileUploadEvent(ChunkUploadedEventDTO
// event) {
// try {
// log.info("Processing chunk: {}/{} for video: {}", event.getChunkIndex(),
// event.getObjectName(),
// event.getVideoPath());

// CompletableFuture<String> visionFuture = CompletableFuture
// .supplyAsync(() -> visionService.describeVideoChunk(event.getVideoPath()),
// executorService);

// CompletableFuture<String> transcriptionFuture = CompletableFuture
// .supplyAsync(() -> transcriptionService.transcribe(event.getAudioPath()),
// executorService);
...
// log.info("Updating MediaChunk with results for chunk: {}",
// event.getChunkId());
// embeddingService.generateAndSaveEmbedding(event, description, transcription);

// } catch (Exception e) {
// log.error("Error handling file upload event for object: {}",
// event.getObjectName(), e);
// }
